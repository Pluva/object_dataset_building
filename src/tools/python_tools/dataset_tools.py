...
def analyse_images_dataset(src_path, verbose=True):
    """
        ...
        for entry in it:
            if not entry.name.startswith('.') and entry.is_file():
                ...
                if is_valid:
                    ...
                    lbl = _process_image_label(entry.name)
                    lbls.append(lbl)

    if verbose:
        print('-Dataset Analysis:')
        print('-- nb_images= {}, avg_size= ({},{}), min_size= ({},{}), max_size= ({},{})'.format(
            len(sizes_x), np.average(sizes_x), np.average(sizes_y), np.min(sizes_x), np.min(sizes_y), np.max(sizes_x), np.max(sizes_y)))
        print('-- avg_label= {}, min_lbl= {}, max_lbl= {}, median_lbl= {}'.format(
            np.average(lbls), np.min(lbls), np.max(lbls), np.median(lbls)))
        np_lbls = np.array(lbls)
 
        # print(len(np.where(np_lbls > 0.9)[0]))
...
def _process_image_label(img_name):
    """
        ...
        :rtype: float
    """
    return float(img_name.split('_')[4])/1000
...
if __name__ == '__main__':
    ...
    parser.add_argument('-m', '--mode', type=str, help='Script mode: RelocateSmallImages, RelocateBigImages, CreateDatasetRootFile, AnalyseDataset.', choices=['rsi', 'rbi', 'cdrf', 'ad'])
    ...
    if args.mode == 'rsi':
        ...
        print('Entering ad mode (Analyse Dataset).')
        analyse_images_dataset(src_path=args.dir, verbose=args.verbose)